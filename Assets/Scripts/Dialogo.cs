   ...
   IEnumerator Bloque2()
    {
        ...
        yield return StartCoroutine(EscribirLento());
        frase = "Nos pilló a todos por sorpresa, la mujer estaba como un roble… Fue un accidente en un Safari, muy aventurera ella.";
        yield return StartCoroutine(EscribirLento());
       frase = "Al intentar hacerse una foto se cayó a la fosa de los cocodrilos ¡no dejaron ni los huesos! En el entierro tuvimos que mantener el ataúd cerrado claro…";
        yield return StartCoroutine(EscribirLento());
        if (emocion == "neutral")
        {
        ...
        {
            frase = "Debería haber más control en esos sitios! Yo voto por que se deshagan de los cocodrilos";
            exito += 0.0f;
        }
        else if (emocion == "triste")
        {
    ...
    IEnumerator Bloque3()
    {
        ...
        yield return StartCoroutine(EscribirLento());
        frase = "Mi novio intentó pedirme matrimonio hace unos años, pero lo hizo con un anillo de lo más básico, ¡Ni hablar! ";
        yield return StartCoroutine(EscribirLento());
        frase = "Una mujer como yo necesita un diamante tan brillante como ella, así que le hice volver a preguntarme cuándo tuviese el valor de comprar uno.";
        yield return StartCoroutine(EscribirLento());
        if (emocion == "neutral")
        {
        ...
        yield return StartCoroutine(EscribirLento());
        frase = "Recuerdas a mi prometido, verdad? Se que en el instituto se metía contigo, pero ahora es todo un amor ¡Ya ni siquiera insulta a minorías cuando nos los cruzamos por la calle!";
        yield return StartCoroutine(EscribirLento());
        frase = "aunque bueno, alguna vez se le escapa algún comentario. Pero todos tenemos defectos, no? Nadie es perfecto en esta vida";
        yield return StartCoroutine(EscribirLento());
         if (emocion == "neutral")
        {
            frase = "Bueeeeno puede que tenga más defectos que la mayoría, pero también tiene más dinero que la mayoría";
            exito -= 0.0f;
        }
        else if (emocion == "feliz")
        {
        ...
        {
            frase = "No sabía que te habías vuelto tan activista, el tiempo fuera de la ciudad te ha cambiado…";
            exito -= 0.0f;
        }
        else if (emocion == "triste")
        {
    ...
    {
        escribiendoBloque = true;
        frase = "Madre mía, ¡cómo están los precios! He venido a por tres tonterías y me voy a gastar un pastizal.";
        yield return StartCoroutine(EscribirLento());
        ...
        yield return StartCoroutine(EscribirLento());
        frase = "Todo por culpa de ese partido! Esos comunistas están arruinando el país con sus locuras sobre la igualdad y nosequé derechos.";
        yield return StartCoroutine(EscribirLento());
        frase = "¿Por qué debería pagar yo más para que un pobre desgraciado tenga su paguita? ¡Que se hubiese esforzado tanto como yo!";
        yield return StartCoroutine(EscribirLento());
         if (emocion == "neutral")
        {
            frase = "No es que lo diga por tí, eh? Aunque visto lo visto tu tampoco te has esforzado mucho en la vida";
            exito -= 0.0f;
        }
        else if (emocion == "feliz")
        {
           frase = " ¡Por fin alguien reconoce mi trabajo duro! Me costó mucho conseguir que me enchufaran en la empresa";
           exito += 0.0f;
        }
        else if (emocion == "enfadado")
        {
        ...
        {
           frase = "¿Te sientes ofendido? Menuda generación se està quedando, todos unos sensibles";
           exito -= 0.0f;
        }
        else if (emocion == "sorprendido")
        {
    ...
    IEnumerator Bloque5()
    {
        ...
        yield return StartCoroutine(EscribirLento());
        frase = " El equipo del fútbol quiere organizar un torneo la misma semana que la asociación de atletismo tiene una competición, se ve que ambos bandos dicen que fueron ellos quien lo dijeron primero…";
        yield return StartCoroutine(EscribirLento());
        frase = "Normalmente no me interesan los dramas del populacho,¡pero quien ignoraría tremendo culebrón!";
        yield return StartCoroutine(EscribirLento());
         if (emocion == "neutral")
        {
            frase = "No sabía que eras demasiado superior como para interesarte en los asuntos del pueblo";
            exito -= 0.0f;
        }
        else if (emocion == "feliz")
        {
        ...
        yield return StartCoroutine(EscribirLento());
        frase = "Bueno, esto seguro que te sorprende. Resulta que los presidentes de ambos clubes ¡Están casados! ";
        yield return StartCoroutine(EscribirLento());
        frase = "Menudo ambiente tiene que haber en esa casa… Són los padres de Y, ¿la recuerdas?";
        yield return StartCoroutine(EscribirLento());
        frase = "Voy con ella a pilates y dice que la cosa está desmadrándose, aunque mi teoría es que es todo una farsa para publicitar los eventos.";
        yield return StartCoroutine(EscribirLento());
         if (emocion == "neutral")
        {
            frase = "e estoy contando información altamente secreta, podrías al menos fingir un poco de emoción";
            exito -= 0.0f;
        }
        else if (emocion == "feliz")
        {
